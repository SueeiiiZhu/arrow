...
- **未闭合的 v2 边界**：
    ...
    2. **init-esc 6pp 缺口（2026-05-22 部分缩小）**：Phase 1 加了 isolated path 拒绝（两端 facing rays 都不挡任何 path，前 5 条 path 宽限期），Phase 2 `canPick` 改为偏好 blocker 数最多的 facing。两个 intent-correct 改动把 init-esc median 从 18 % → 15 %，关闭 ~3pp / 10pp gap。剩下的 6pp 与 path-len p90 同根：corpus path-len p90 27，我们 16；path 短 → head 离边沿近 → 常常朝外出界（射线空 = init-escapable）。partition 原语本身限制了这条路；要彻底闭合需要改 path generation（exponential 长尾），而那会与 chainDepth 目标冲突。
    3. **path-len p90 16 vs corpus 27**：把 `--max-arrow-len` 默认值从 12 调到 18 后，25×31 上 p50 已对齐（8 vs 8），但 p90 还短一截。partition 原语本身把单条 path 的上限切在 maxLen；要让出 corpus 那种偶尔出现的 40+ 长度的蛇型箭头，要么再抬高 maxLen（实测 24 时 chainDepth 会从 10 退到 8，得不偿失），要么换成对单条 path 做长尾偏置（exponential 而不是 uniform）。当前 sweet spot 是 maxLen=18。
