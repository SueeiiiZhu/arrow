...
- **未闭合的 v2 边界**：
    ...
    2. **path-len p90 17 vs corpus 27（2026-05-22 缩小）**：把 `--max-arrow-len` 默认值从 12 调到 18 后，25×31 上 p50 已对齐（8 vs 8），但 p90 还短一截。partition 原语本身把单条 path 的上限切在 maxLen；要让出 corpus 那种偶尔出现的 40+ 长度的蛇型箭头，要么再抬高 maxLen（实测 24 时 chainDepth 会从 10 退到 8，得不偿失），要么换成对单条 path 做长尾偏置（exponential 而不是 uniform）。当前 sweet spot 是 maxLen=18。
